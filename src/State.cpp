...
std::string GetConfigFilename()
{
	...
	if ( !pcHome ) pcHome = ".";
	return std::string(pcHome) + "/Library/Preferences/OpenMortal.cfg";
#else
	const char* pcHome = getenv("HOME");
	if ( !pcHome ) pcHome = ".";
	return std::string(pcHome) + "/.openmortalrc";
#endif
}
...
SState::SState()
{
	...
	switch ( iLangID )
	{
	case 0x0e: pcLang = "hu"; break;
	case 0x0c: pcLang = "fr"; break;
	case 0x0a: pcLang = "es"; break;
	default:   pcLang = "en"; break;
	}

	strcpy( m_acLanguage, pcLang );
	...
	if ( iResult )
	{
		...
		const char* pcUser = getenv("USER");
		if ( pcUser )
		{
			strncpy( m_acNick, pcUser, 127 );
			m_acNick[127] = 0;
		}
	}
#endif
...
{
	if ( strcmp( m_acLanguage, a_pcLanguage ) != 0 )
	{
...
//	poSv = get_sv("", FALSE); if (poSv) m_ = SvIV( poSv );
	...
	poSv = get_sv("MUSICMODE", FALSE);     if (poSv) m_enMusicMode = (TMusicModeEnum) SvIV( poSv );
	poSv = get_sv("LEVELSELECT", FALSE);   if (poSv) m_enLevelSelect = (TLevelSelectEnum) SvIV( poSv );
	poSv = get_sv("SELECTEDLEVEL", FALSE); if (poSv) m_iSelectedLevel = SvIV( poSv );
...
//	oStream << "=" << m_ << '\n';
	...
	oStream << "MUSICMODE=" << m_enMusicMode << '\n';
	oStream << "LEVELSELECT=" << m_enLevelSelect << '\n';
	oStream << "SELECTEDLEVEL=" << m_iSelectedLevel << '\n';
