...
int Game::mg_iResolvedLevel    = 0;

static const int g_aiGoodLevels[] = {1, 2, 5, 6, 7, 10};
static const int g_aiEvilLevels[] = {3, 4, 8, 9, 11, 12};
...
Game::Game( bool a_bIsReplay, bool a_bWide, bool a_bDebug)
{
	...
	{
		mg_iBackgroundNumber = ResolveLevel();
	}
	mg_iResolvedLevel = 0;	// reset cache for next game
...
void Game::Draw()
{
	...
	m_poBackground->AdvanceAnimation();
	DrawBackground();
...
int Game::ResolveLevel()	//static
{
	if ( mg_iResolvedLevel != 0 ) return mg_iResolvedLevel;

	switch ( g_oState.m_enLevelSelect )
	{
	case SState::Level_RANDOM:
		mg_iResolvedLevel = rand() % 12 + 1;
		break;
	case SState::Level_GOOD:
		mg_iResolvedLevel = g_aiGoodLevels[ rand() % 6 ];
		break;
	case SState::Level_EVIL:
		mg_iResolvedLevel = g_aiEvilLevels[ rand() % 6 ];
		break;
	case SState::Level_SELECT:
		mg_iResolvedLevel = g_oState.m_iSelectedLevel;
		break;
	default:	// Level_NATURAL
		mg_iResolvedLevel = mg_iBackgroundNumber;
		break;
	}
	return mg_iResolvedLevel;
}
