...
//	int perl_argc = 3;
	...
	// Windows leg (see the -I below on the #ifdef _WIN32 side) and the
	// same fix: point -I explicitly at a bundled copy. $APPDIR is set by
	// linuxdeploy's generated AppRun to the AppImage's mount point (see
	// GetDataDir() in main.cpp, which already relies on it) and is unset
	// for the deb/rpm/flatpak builds, where the system/runtime Perl's own
	// default @INC already covers this -- so this only ever engages
	// inside the AppImage.
	std::string sPerlLib;
