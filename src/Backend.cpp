...
{
	char acBuffer[2048];
	int iRem = (int)sizeof(acBuffer);
	int iNumChars = snprintf( acBuffer, iRem, "%d %d  %d %d %d %d  %d %d %d %d  %d  ",
		m_iBgX, m_iBgY,
		...
		m_iNumDoodads );
	if ( iNumChars < 0 || iNumChars >= iRem ) { a_rsOutString = acBuffer; return; }

	int i;
	for ( i = 0; i<m_iNumDoodads; ++i )
	{
		SDoodad& roDoodad = m_aoDoodads[i];
		int n = snprintf( acBuffer+iNumChars, iRem-iNumChars, "%d %d %d %d %d %d %d %s  ",
			roDoodad.m_iX, roDoodad.m_iY, roDoodad.m_iType, roDoodad.m_iFrame,
			roDoodad.m_iDir, roDoodad.m_iGfxOwner,
			(int)roDoodad.m_sText.size(), roDoodad.m_sText.c_str() );
		if ( n < 0 || n >= iRem-iNumChars ) { a_rsOutString = acBuffer; return; }
		iNumChars += n;
	}

	int n = snprintf( acBuffer+iNumChars, iRem-iNumChars, "%d ", m_iNumSounds );
	if ( n < 0 || n >= iRem-iNumChars ) { a_rsOutString = acBuffer; return; }
	iNumChars += n;
	for ( i = 0; i<m_iNumSounds; ++i )
	{
		n = snprintf( acBuffer+iNumChars, iRem-iNumChars, " %d %s",
			(int)m_asSounds[i].size(), m_asSounds[i].c_str() );
		if ( n < 0 || n >= iRem-iNumChars ) { a_rsOutString = acBuffer; return; }
		iNumChars += n;
	}
